...
unsigned char *MVCamera::g_pBayerBuffer[BAYER_BUFSIZE] = {NULL, NULL, NULL, NULL};
unsigned char *MVCamera::g_pRgbBuffer[RGB_BUFSIZE] = {NULL, NULL, NULL, NULL};  //处理后数据缓存区
...
void MVCamera::Process() {
  while (!stopped) {
    ...
    CameraImageProcess(hCamera, g_pBayerBuffer[bayer_id], g_pRgbBuffer[rgb_id], &sFrameInfo);
...
int MVCamera::GetFrame(Mat &frame)
{
  ...
  memcpy(frame.data, g_pRgbBuffer[rgb_buf_tail],frame.cols * frame.rows * channel);
